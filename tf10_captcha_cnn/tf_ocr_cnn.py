...
def tf_ocr_train(train_method, train_step, result_process, method='train'):
    ...
    img_batch, label_batch, cnt_batch = tf.train.shuffle_batch([img, label, cnt], batch_size=100, capacity=3000, min_after_dequeue=200, num_threads=2)
    #use full batch size
    ...
    #img_t_batch, label_t_batch, cnt_t_batch = tf.train.shuffle_batch([img_t, label_t, cnt_t], batch_size=10, capacity=500, min_after_dequeue=200, num_threads=1)
    img_t_batch, label_t_batch, cnt_t_batch = tf.train.batch([img_t, label_t, cnt_t], batch_size=10, capacity=10)
    if method == 'train':
