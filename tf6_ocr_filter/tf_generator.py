...
IMG_IN_DIR = './source/img_in'
IMG_OUT_DIR = './source/img_out'
...
def random_image_generator(idx, label, path):
    ...
    retval, b_img = cv2.threshold(total_img, 20, 255, cv2.THRESH_BINARY)
    cv2.imwrite(IMG_OUT_DIR + '/' + str(idx) + '_' + ''.join(label) + '.png', b_img)
    #gassuian noise
    ...
    cv2.imwrite(IMG_IN_DIR + '/' + str(idx) + '_' + ''.join(label) + '.png', total_img)
